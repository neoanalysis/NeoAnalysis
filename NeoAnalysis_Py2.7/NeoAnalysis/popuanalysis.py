...
class PopuAnalysis():
    ...
    def __spike_normalize(self,data):
        return_data = list()
        for ite in data:
            max_data = max([value.max() for _,value in ite.iteritems()])
            min_data = min([value.min() for _,value in ite.iteritems()])
            norm_data = {key:(value-min_data)/(max_data-min_data) for key,value in ite.iteritems()}
    ...
    def plot_spike(self,store_key,conditions,normalize=False,fig_mark=[0],line_style=False,x_lim=False,y_lim=False,err_style='ci_band',ci=68):
        ...
        '''
        fr_time = self.store[self.store.keys()[0]][store_key]['time'].value
        if isinstance(normalize,list):
            ...
            for ite_file in self.store.keys():
                data_temp = dict()
                for ite_cond in self.store[ite_file][store_key]['data'].keys():
        ...
        fr_max = max([v.max() for _,v in target_mean.iteritems()])
    ...
    def plot_analog(self,store_key,conditions,line_style=False,fig_mark=[0],x_lim=False,y_lim=False):
        ...
        '''
        times = self.store[self.store.keys()[0]][store_key]['time'].value
        datas_all = list()
        for ite_file in self.store.keys():
            data_temp = dict()
            for ite_cond in conditions:
