...
class transfile():
    ...
    Data structure in the converted HDF5 file:
        ...
        events: contain data values and timestamps
        comments: contain data values and timestamps
    Args
        filename (string):
    ...
    def __init__(self,filename,machine_type,replace=False,**arg):
        ...
        if machine_type == 'tdt':
            # not tested
            self.__transTdt(**arg)
        if machine_type == 'spike2':
            # not tested
            self.__transSpike2(**arg)
        if machine_type == 'neuroexplorer':
            # not tested
            self.__transNeuroexplorer(**arg)
    ...
    def __transTdt(self):
        # not tested
        r = io.TdtIO(dirname=self.filename)
        bl = r.read_block(lazy=False,cascade=True)
        seg = bl.segments[0]
        # spikes entity
        spikes = dict()
        spikes['spikes'] = dict()
        for spk in seg.spiketrains:
            chan = spk.annotations['channel_index']
            unit = spk.annotations['unit_id']
            time = np.array(spk.times.rescale(pq.ms))
            waveform = np.array(spk.waveforms.rescale(pq.mV))
            waveform.shape = waveform.shape[0],waveform.shape[-1]
        ...
        events['neo_events'] = dict()
        for evt in seg.events:
            key = evt.annotations['channel_index']
            times = np.array(evt.times.rescale(pq.ms))
            labels = np.array(evt.labels)
            events['neo_events'][key] = {'times':times, 'labels':labels}
    ...
    def __transSpike2(self):
        # not tested
        r = io.Spike2IO(filename = self.filename+'.smr')
        seg = r.read_segment(lazy = False, cascade=True)
        # spikes entity
        spikes = dict()
        spikes['spikes'] = dict()
    ...
    def __transNeuroexplorer(self):
        # not tested
        r = io.NeuroExplorerIO(filename=self.filename + '.nex')
        seg = r.read_segment(lazy=False, cascade=True)
        # spikes entity
        spikes = dict()
        spikes['spikes'] = dict()
        for spk in seg.spiketrains:
            chan = spk.annotations['channel_index']
            # lack unit_index
            # unit = spk.annotations['unit_index']
            time = np.array(spk.times.rescale(pq.ms))
            waveform = np.array(spk.waveforms.rescale(pq.mV))
            waveform.shape = waveform.shape[0],waveform.shape[-1]
            # key = 'spike_'+str(chan)+'_'+str(unit)
            # spikes['spikes'][key] = {'times':time,'waveforms':waveform}
            # lack unit_index. when having testing data, add this information
            key = 'spike_'+str(chan)
            spikes['spikes'][key] = {'times':time,'waveforms':waveform}
        ...
        if len(keys_in) > 0:
            ...
            with hp.File(self.trans_file,'a') as f:
                for ite_k,ite_v in flat_data.items():
                    if ite_k in f:
                        del f[ite_k]
                    if isinstance(ite_v,np.ndarray) and str(ite_v.dtype)[1]=='U':
                        ite_v = np.array(ite_v,dtype=str(ite_v.dtype).replace('U','S'))
                    if isinstance(ite_v,list):
                        ite_v = [iite_v.encode('ascii','ignore') if isinstance(iite_v,str) else iite_v for iite_v in ite_v]
                    f[ite_k] = ite_v
                f.flush()
...
def generate_comments(filename, method, replace=False, **arg):
    '''
    method:
        'move':move data from events to comments
        'map': map data in events to comments
    '''
    if method == 'move':
        ...
        with hp.File(filename,'a') as f:
            ...
            if len(arg['mapping'])>0:
                comments_times = []
                comments_labels = []
                for map_key, to_value in arg['mapping'].items():
